...
from muse.utils.transform_utils import quat2mat, mat2quat, euler2mat, quat_difference, quat_multiply, \
    ...
    def get_action(self, model, observation, goal, **kwargs):
        ...
        if self.sticky_gripper:
            ...
            if buttons['rightTrig'][0] > 0.75 and not self._trig_pressed:  # mostly closed
                ...
                self._targ_gripper = gripper = 0. if curr_gripper.item() > self._gripper_max / 3 else self._gripper_max
        ...
        desired_euler = quat2euler(desired_quat)
        ...
        return self._postproc_fn(model, observation, goal, AttrDict.from_dict({
            ...
            'target': {
                ...
                'ee_orientation_eul': quat2euler(desired_quat),
