...
from muse.envs.robosuite.robosuite_env import RobosuiteEnv, get_ordered_objects_from_arr
...
class WaypointPolicy(Policy):
    ...
    def get_action(self, model, observation, goal, **kwargs):
        ...
        # compute the action
        dpos = wp_pose[:3] - pos
        target_q = T.euler2quat(wp_pose[3:])
        curr_q = T.euler2quat(ori)
...
def get_tool_hang_policy_params(obs, goal, env=None, random_motion=True):
    ...
    obj2_yaw = T.quat2euler(obj2_q)[2]
