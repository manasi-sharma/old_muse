...
class SoftDTWLoss(torch.nn.Module):
    ...
    def __init__(self, gamma=1.0, spatial_independent=False, bandwidth=None, dist_fn=None):
        '''
        ...
        # default is mean L1 distance
        self.dist_fn = dist_fn if dist_fn else (lambda x, y, dim=None: (x - y).abs().mean(dim) if dim is not None else (x - y).abs())
    ...
    def forward(self, x, y):
        ...
        Arguments:
            x (torch.Tensor): Time series data of size (batch_dim x x_time_dim x space_dim)
            y (torch.Tensor): Time series data of size (batch_dim x y_time_dim x space_dim)

        Returns:
            loss (torch.Tensor): Loss for each data point in batch. Size = batch_dim
        '''
        x = combine_after_dim(x, 2)
        y = combine_after_dim(y, 2)
        return SoftDTWLossFunction.apply(x, y, (self.gamma, self.spatial_independent, self.bandwidth, self.dist_fn))
...
class SoftDTWLossFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, y, params):
        ...
        Arguments:
            ctx : context
            x (torch.Tensor) : tensor of dimensions (batch_dim x x_time_dim x space_dim)
            y (torch.Tensor) : tensor of dimensions (batch_dim x y_time_dim x space_dim)
        ...
        gamma, spatial_independent, bandwidth, dist_fn = params
        ctx.gamma = gamma
        ...
        x_batch_dim, x_time_dim, x_space_dim = x.shape
        y_batch_dim, y_time_dim, y_space_dim = y.shape
        ...
        D = torch.zeros(D_dims, device=ctx.device)
        R = torch.ones(R_dims, device=ctx.device) * inf
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.spatial_independent:
            ...
            G = jacobean_product_squared_euclidean(x, y, E[:, 1:-1, 1:-1])
...
if __name__ == '__main__':
    a = torch.zeros((10, 20, 3))
    b = torch.zeros((10, 20, 3))

    loss = SoftDTWLoss()

    out = loss.forward(a, b)
    print(out)
