...
class RSDepthCamera(Sensor):
    def _init_params_to_attrs(self, params: AttrDict):
        ...
        if self.series == RSSeries.sr300:
            ...
            self.check_W = get_with_default(params, "check_width", self.W)
            self.check_H = get_with_default(params, "check_height", self.H)
            assert self.W == self.check_W, f"Width: Expected {self.check_W} but got {self.W}"
            assert self.H == self.check_H, f"Height: Expected {self.check_H} but got {self.H}"
