"""
Success functions we use for evaluating.
"""
import numpy as np
...
def circular_difference_fn(a, b):
...
def move_object_to_position_and_angle_reward(trajectory_obs, done_idxs, goal_obs, l2_tol=10.,
                                             l2_angle_tol=np.deg2rad(10.), pos_weight=0.75):
    ...
    l2_all_angle = get_l2_then_mean(trajectory_obs, goal_obs_rep, 'block_angles', diff_fn=circular_difference_fn,
                                    keepdims=True)
    # print((l2_all_angle < l2_angle_tol).sum())
    rew = pos_weight * (l2_all_dist < l2_tol).astype(float) + (1 - pos_weight) * (l2_all_angle < l2_angle_tol).astype(
        float)
...
def get_move_object_to_position_and_angle_reward(l2_tol=15., l2_angle_tol=np.deg2rad(15.), pos_weight=0.75):
    return lambda tobs, didx, gobs: move_object_to_position_and_angle_reward(tobs, didx, gobs, l2_tol=l2_tol,
                                                                             l2_angle_tol=l2_angle_tol,
