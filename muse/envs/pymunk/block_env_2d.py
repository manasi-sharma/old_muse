...
from muse.utils.geometry_utils import Maze
from muse.utils.input_utils import UserInput
from muse.utils.pygame_utils import PygameDisplayWrapper
...
class BlockEnv2D(Env):
    ...
    A randomly generated maze, with pymunk physics. some blocks will be created,
    which are dynamic but cannot move between maze walls

    you control the RED block, which can move freely between walls.
    Colliding with a dynamic block will cause it to move!
    ...
    def _init_params_to_attrs(self, params):
        ...
        if self._action_noise_prob > 0:
            logger.debug(
                f"Adding action noise std={np.rad2deg(self._action_noise_theta)} with prob={self._action_noise_prob}")
        ...
        if self._block_size_upper is not None:
            ...
            assert np.all(self._block_size_upper >= self._block_size_lower), [self._block_size_lower, "not <=",
                                                                              self._block_size_upper]
    ...
    def create_world(self, presets: AttrDict = AttrDict()):
        ...
        if self.maximize_agent_block_distance:
            ...
            candidate_agent_positions = np.random.uniform(low=np.array([0.0, 0.0]), high=self.grid_size,
    ...
    def step(self, action):
        ...
        if np.any(action.action[0, 2:] > 0):
            ...
            if self._grab_one_only:
                ...
                thresh = self._block_grabbing_frac * np.maximum(1.4 * np.max(self.ego_block_size),
                                                                np.max(self.all_block_sizes[block_id_closest]))
        ...
        while x < effective_dt:
            ...
            if self._break_constraints_on_large_impulse:
                for i in range(len(self.bodies)):
                    if self.added_constraints[i] is not None:
                        total_cstr_impulse = self.added_constraints[i].impulse + sum(
                            cstr.impulse for cstr in self.extra_constraints[i])
                    ...
                    for i in range(len(self.static_lines)):
                        if len(coll_points[i]) > 0:
                            for j in range(len(coll_points[i])):
                                ...
                                vel = vel - max(vel.dot(collision_direction), 0) * collision_direction / (
                                            np.linalg.norm(collision_direction) ** 2)
        ...
        if not self.disable_images:
            if self.user_input is None:
                # flush the event queue if no ui provided but pymunk is initialized
                pygame.event.get()
    ...
    def _get_obs(self):
        ...
        for i in range(self.num_blocks):
            ...
            if block_contact[i]:
                # first point
                block_contact_points[i] = info.points[0].point_b.x, info.points[0].point_b.y, info.points[0].point_a.x, \
                info.points[0].point_a.y
    ...
    def get_default_env_spec_params(params: AttrDict = None) -> AttrDict:
        disable_imgs = get_with_default(params, "disable_images", False)
        grid_size = get_with_default(params, "grid_size", np.array([600., 600.]))
        img_height, img_width = get_with_default(params, "image_size", grid_size.astype(int))
        num_blocks = get_with_default(params, "num_blocks", 10, map_fn=int)
        num_maze_cells = get_with_default(params, "num_maze_cells", 5, map_fn=int)
