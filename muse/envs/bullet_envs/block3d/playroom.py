...
from muse.utils.transform_utils import euler2quat_intrinsic, quat_difference, convert_rpt, convert_quat_to_rpt
...
class DrawerPlayroomEnv3D(BlockEnv3D):
    default_params = BlockEnv3D.default_params & d(
        ...
        reset_full_every_n_resets=1,

        object_friction_coef=1000.,
        object_other_friction_coef=0.1,
        object_linear_damping=0.04,
        object_angular_damping=0.04,
        object_contact_stiffness=10000,
        object_contact_damping=200,

        random_init_snap_drawer=True,
        random_init_snap_cabinet=True,
    ...
    def _load_asset_objects(self, presets: d = d()):
        ...
        self.cab_door_id = p.loadURDF(self.cab_door_file, globalScaling=0.8,
                                      ...
                                      baseOrientation=euler2quat_intrinsic(np.array([0., 0., np.pi])).tolist(),
                                      physicsClientId=self.id)
        ...
        if self._use_buttons:
            ...
            self._button_positions = [self.cab_center + button_offset, self.cab_center + button_offset + lateral_offset,
                                      self.cab_center + button_offset - lateral_offset]

            for i in range(3):
                p.resetBasePositionAndOrientation(self.buttons[i], self._button_positions[i],
                                                  [0, 0, 0, 1], physicsClientId=self.id)
                cid = p.createConstraint(self.buttons[i], -1, -1, -1, p.JOINT_FIXED, [0, 0, 0], [0, 0, 0],
                                         self._button_positions[i], physicsClientId=self.id)
            ...
            if self._use_buttons:
                for i in range(len(self.buttons)):
                    if self.is_button_pressed(i):
                        ...
                        p.applyExternalForce(self.buttons[i], -1, [0, 0, -500. * dz], [0, 0, 0], p.LINK_FRAME,
                                             physicsClientId=self.id)
            # f = np.zeros(3)
            # for pt in cpts:
    ...
    def get_default_env_spec_params(params=None):
        base_prms = BlockEnv3D.get_default_env_spec_params(params)
        use_buttons = get_with_default(params, "use_buttons", False)
        return base_prms & d(
            names_shapes_limits_dtypes=base_prms.names_shapes_limits_dtypes + [
                ...
                ("drawer/contact", (1,), (False, True), bool),  #
                ("drawer/closed", (1,), (False, True), bool),  #
                ...
                ("cabinet/contact", (1,), (False, True), bool),  #
                ("cabinet/closed", (1,), (False, True), bool),  #
                ...
                ("buttons/closed", (3,), (False, True), bool),  #
                ("buttons/contact", (3,), (False, True), bool),  #
            ],
            ...
            ] + (['buttons/position', 'buttons/joint_position', 'buttons/closed',
                  'buttons/contact'] if use_buttons else []),
            param_names=base_prms.param_names + ["cabinet/size", "cabinet/handle_size"]
        )


class DrawerButtonPlayroomEnv3D(DrawerPlayroomEnv3D):
    default_params = DrawerPlayroomEnv3D.default_params & d(
        use_buttons=True,
    )
...
if __name__ == '__main__':
    ...
    cls = DrawerButtonPlayroomEnv3D if args.use_buttons else DrawerPlayroomEnv3D
    env = make(cls, params)
