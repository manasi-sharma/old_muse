...
from muse.envs.bullet_envs.robot import BulletRobot
...
from muse.envs.bullet_envs.utils_env import RobotControllerMode, get_view
...
from muse.policies.controllers.robot_config import os_torque_control_panda_cfg
from muse.envs.bullet_envs import control_utils
from muse.utils.geometry_utils import CoordinateFrame, world_frame_3D, clip_ee_orientation_conical
from muse.envs.bullet_envs.pybullet_utils import draw_cyl_gui, draw_point_gui
...
class BlockEnv3D(RobotBulletEnv):
    ...
    def _load_robot(self, presets: d = d()):
        ...
        if self.robot_controller_mode == RCM.xddot_pid:
            self.robot.create_controller(self.x_controller_params["type"], "x", self.x_controller_params)
            self.robot.create_controller(self.posture_q_controller_params["type"], "posture_q",
                                         self.posture_q_controller_params)
            self.robot.create_controller(self.xdot_controller_params["type"], "xdot",
            ...
            self.robot.create_controller(self.robot_controller_params["type"], "x",
                                         self.robot_controller_params)
        elif self.robot_controller_mode == RCM.xdot_pid:
            # TODO actually use this
            self.robot.create_controller(self.robot_controller_params["type"], "xdot",
                                         self.robot_controller_params)
        elif self.robot_controller_mode in [RCM.xddot_with_force_pid, RCM.xddot_with_zero_force_pid]:
            self.robot.create_controller(self.force_controller_params["type"], "force",
    ...
    BLOCK_LOW = (0.025, 0.025, 0.025)
    BLOCK_HIGH = (0.055, 0.055, 0.055)
    BLOCK_MASS_RANGE = (0.05, 0.15)
    BLOCK_ROTATION_BOUNDS = (-np.pi / 4, np.pi / 4)
    MUG_SCALES = (0.05, 0.09)
    MUG_MASSES = (0.01, 0.05)
    MUG_ROTS = (-np.pi, np.pi)

    default_params = RobotBulletEnv.default_params & d(
        skip_n_frames_every_step=5,
        time_step=0.02,
        num_blocks=1,
    ...
    def get_default_env_spec_params(params=None):
        ...
        prms = d(
            ...
            names_shapes_limits_dtypes=[
                ...
                ('finger_left_contact', (1,), (False, True), bool),
                ('finger_right_contact', (1,), (False, True), bool),
...
if __name__ == '__main__':
    ...
    cls = MugEnv3D if args.use_mug else BlockEnv3D
    env = make(cls, params)
